...
namespace SFO2O.Framework.Cache
{
    public class RedisCacheHelper
    {
        ...
        public static T AutoCache<T>(string keyPrefix, string key, Func<T> method, int cacheMinutes = 0)
        {
            if (cacheMinutes == 0)
            {
                cacheMinutes = ConfigHelper.SessionExpireMinutes;
            }
            var temp = Get<T>(keyPrefix + key);
            ...
            if (temp == null || EqualityComparer<T>.Default.Equals(temp, default(T)))
            {
                ...
                if (temp1 != null)
                {
                    Add(keyPrefix + key, temp1, DateTime.UtcNow.AddMinutes(cacheMinutes));
