...
namespace SFO2O.Supplier.Common
{
    ...
    public static class LTImage
    {
        ...
        /// <param name="isSetInterpolationMode">是否使用高质量插值算法</param>
        /// <param name="isHighQuality">是否使用高质量呈现模式</param>
        /// <param name="isKeepSize">是否保持图片的尺寸</param>
        /// <returns>返回一个Bitmap类型的图片实例</returns>
        public static Bitmap ResizeImage(System.Drawing.Image imgOrigImage, int nNewWidth, int nNewHeight, bool isSetInterpolationMode, bool isHighQuality = true, bool isKeepSize = false)
        {
            ...
            if (isKeepSize)
            {
                bmpTmpBitmap = new Bitmap(nNewWidth, nNewHeight);
            }
            else
            {
                bmpTmpBitmap = new Bitmap(nWidth, nHeight);
            }
            ...
            if (isKeepSize)
            {
                destRect = new Rectangle((nNewWidth - nWidth) / 2, (nNewHeight - nHeight) / 2, nWidth, nHeight);
            }
            else
            {
                destRect = new Rectangle(0, 0, nWidth, nHeight);
            }
            gTmpGraphics.DrawImage(imgOrigImage, destRect, new Rectangle(0, 0, imgOrigImage.Width, imgOrigImage.Height), GraphicsUnit.Pixel);
        ...
        public static Bitmap ResizeImage_Store(System.Drawing.Image imgOrigImage, int nNewWidth, int nNewHeight, bool isSetInterpolationMode, bool isHighQuality = true)
        {
            //var nWidth = imgOrigImage.Width < nNewWidth ? imgOrigImage.Width : nNewWidth;
            //var nHeight = imgOrigImage.Height < nNewHeight ? imgOrigImage.Height : nNewHeight;
            ...
            Bitmap bmpTmpBitmap = new Bitmap(imgOrigImage, dw, dh);
            Graphics gTmpGraphics = Graphics.FromImage(bmpTmpBitmap);
            ...
            if (isHighQuality)
                ...
                gTmpGraphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.HighSpeed;
            gTmpGraphics.Clear(Color.Transparent);
            gTmpGraphics.DrawImage(imgOrigImage, new Rectangle(x, y, dw, dh), new Rectangle(0, 0, ow, oh), GraphicsUnit.Pixel);
            gTmpGraphics.Dispose();
