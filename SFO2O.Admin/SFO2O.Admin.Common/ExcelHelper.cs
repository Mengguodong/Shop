...
namespace SFO2O.Admin.Common
{
    public class ExcelHelper
    {
        ...
        public void SetBTitle(ISheet sheet, XSSFWorkbook workbook, string bigTitle, int colspan, int rowIndex)
        {
            #region 大标题
            IRow itemList = sheet.CreateRow(rowIndex);
            IFont bigTitleFont = workbook.CreateFont();
        ...
        public void SetSubTitle(ISheet sheet, XSSFWorkbook workbook, string subTitle, int rowindex)
        {
            #region 设置每列的小标题
            var titleRow = sheet.CreateRow(rowindex);
            IFont headfont = workbook.CreateFont();
        ...
        /// </summary>
        /// <param name="strFileName">excel文档路径</param>
        /// <param name="startIndex">导入起始行</param>
        /// <returns></returns>
        public static DataTable Import(string strFileName,int startIndex)
        {
            ...
            for (int i = (sheet.FirstRowNum + startIndex); i <= sheet.LastRowNum; i++)
            {
                HSSFRow row = (HSSFRow)sheet.GetRow(i);
                DataRow dataRow = dt.NewRow();
                for (int j = row.FirstCellNum; j < cellCount; j++)
